...
| `journey.c` / `journey.h` | Bottom journey strip. Owns its own slide animations for step changes and sleep transitions. Loads mage sprite frames (`MAGE_IDLE_1/2`, `MAGE_WALK_1/2/3`) in `journey_init` and releases them in `journey_deinit`. The walk-frame index is updated from slide progress in `slide_update` and read by `draw_token` so the mage cycles through walk frames while the token is moving. |
| `widgets.c` / `widgets.h` | Date ribbon (banner + feather + familiar) and stat row (heart + torch + battery %). Owns the banner / feather / torch atlas bitmaps via `widgets_init`/`widgets_deinit`. The ribbon banner is a static 200×28 PNG; the torch sheet is 256×32 with 8 32×32 animation frames (we currently draw frame 0 statically — battery-state variation is still text only). The heart and familiar stay procedural. |
| Resource bundle | Bitmaps live in `resources/` and are declared in `package.json` under `pebble.resources.media` with `IMAGE_*` resource IDs. Sub-bitmaps from spritesheets use `gbitmap_create_as_sub_bitmap(parent, sub_rect)` — destroy sub-bitmaps **before** their parent atlas, and draw alpha-having bitmaps with `graphics_context_set_compositing_mode(ctx, GCompOpSet)` so transparent regions don't paint over what's behind. |
