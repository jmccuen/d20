...
| `dice3d.c` / `dice3d.h` | Procedural 3D dodecahedron (D12, hour die) and pentagonal trapezohedron (D10, minute dice). Euler rotation → orthographic projection → 2D signed-area backface cull → 4-step flat shading. No depth buffer (convex + backface cull = no overdraw). The vertex and face tables are emitted by `tools/gen_dice.py` — re-run that script rather than hand-editing. The script validates edge lengths, kite planarity, opposite-pair value sums, and CCW perimeter ordering (a face listed out of perimeter order renders as a self-intersecting polygon and shows visible angular gaps). |
