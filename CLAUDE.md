...
| `dice3d.c` / `dice3d.h` | Procedural 3D dodecahedron (D12, hour die) and pentagonal trapezohedron (D10, minute dice). Euler rotation → orthographic projection → 2D signed-area backface cull → 4-step flat shading. No depth buffer (convex + backface cull = no overdraw). The vertex tables, face tables, and per-value rest-rotation tables are emitted by `tools/gen_dice.py` — re-run that script rather than hand-editing. The script validates edge lengths, kite planarity, opposite-pair value sums, and CCW perimeter ordering (a face listed out of perimeter order renders as a self-intersecting polygon and shows visible angular gaps). At draw time the per-value rest rotation puts the face labeled `die->value` dead-on the camera; every visible face draws its own baked numeral. Numerals are culled per-face via a point-in-convex-polygon check on the text-box corners — any corner outside the projected face skips the glyph, so tilted side faces don't spill text onto their neighbour. The face whose label matches `die->value` (the "current time" face) is tinted; all other faces use a plain ink color. Both are macros at the top of the file, intended to become user-selectable in the eventual settings page. |
